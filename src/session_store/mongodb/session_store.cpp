...
SmartPointer<MongoStore::SessionStore> MongoStore::SessionStore::Find(const string& id_string)
{
  ...
  id.init(id_string);
  {
    SmartPointer<MongoDB::ResultSet<SessionStore> > results = MongoDB::ResultSet<SessionStore>::prepare(MONGO_QUERY("_id" << id));
...
MongoStore::SessionStore MongoStore::SessionStore::Create(Data params)
{
  ...
  MongoDB::Collection& collection      = Databases::singleton::Get()->get_database<MongoDB::Database>(database)[collection_name];
  mongo::BSONObj       obj;
  {
