...
void CrailsServer::ParseForm(const Server::request&, Response response, Params& params)
{
...
void CrailsServer::ParseResponse(const Server::request& request, Response response, Params& params)
{
  static const Regex is_multipart("^multipart/form-data", REG_EXTENDED);
  string             type       = params["header"]["Content-Type"].Value();

  if (!(is_multipart.Match(type)))
    ParseMultipart(request, response, params);
  else
    ParseForm(request, response, params);
}
